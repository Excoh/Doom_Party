...
public class Player2Bullet : MonoBehaviour {
	...
	void Start() {
		...
		if (MyGlobalController2.SharedInstance.Mode == 0) {
			angle = MyGlobalController2.SharedInstance.JoyAngle + Random.Range (-0.06f, 0.06f);
		} else {
			if (MyGlobalController2.SharedInstance.Mode == 2) { //Sprint
				angle = MyGlobalController2.SharedInstance.JoyAngle + Random.Range (-0.2f, 0.2f);
			} else { // Crouch
				angle = MyGlobalController2.SharedInstance.JoyAngle;
			}
		}
	...
	void OnCollisionEnter2D(Collision2D col)
	{
		...
		if (col.gameObject.tag == "Enemy") {
			Instantiate (Blood, new Vector3 (transform.position.x+speedX/2.0f, transform.position.y+speedY/2.0f,0), Quaternion.identity);
			col.gameObject.GetComponent<EnemyAI>().damage(1, 2);
			Destroy(gameObject);
		}
	}
